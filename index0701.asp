          ...
          new Vue({
            ...
            methods: {
              ...
              async submitForm() {
                ...
                if (this.srlist.length > 0) {
                  ...
                  while (n1 <= n2) {
                    ...
                    s1='<a href="/03_category_info.asp?sn=';
                    //vue中做 class必須去掉,接下來探討的就是html差異性
                    //s1=s1+ recj["SS_NNO"]+" class=''>";                    
                    s1=s1+ recj["SS_NNO"]+'">';     
                    s1=s1+recj['S_NEW_Title'];
                    s1=s1+ '</a>';
                    
                    //console.log(recj['Sub2_S_NEWS_TITLE']);
                    //console.log(recj['SS_SUB_TITLE']);
                    //recj['url1'] = '<a href="/03_category_info.asp?sn=' + recj["SS_NNO"] + ' class="">' + recj['S_NEWS_TITLE'] + '</a>';
                    //console.log(s1);
                    recj['url1']=s1;
                    recj['url2'] = '<a href="/03_category_info.asp?sn=' + recj["SS_NNO"] + '"><span class="">' + recj['Sub2_S_NEWS_TITLE'] + '</span></a>';                  
                    //console.log(recj['url2']);
                    this.srlist[n1] = recj;               
                    n1++;
                  }                
                }              
              ...
              btnpage(ns) {
                ...
                while (n1 <= n2) {
                  j1=this.srlist[n1];
                  //根據data.asp應該只有客製的日期要變
                  if (j1['SS_TTYPE']==1)
                  {
                    j1["S_NEWS_TITLE"] = j1["Sub2_S_NEWS_TITLE"];  
                    j1["S_NEW_Title"]=j1["Sub2_S_NEW_Title"];
                    j1["S_NEW_Special"]=j1["Sub2_S_NEW_Special"];
                    j1["S_NEW_Promotions"]=j1["Sub2_S_NEW_Promotions"];
                    j1["S_NEW_Attach"]=j1["Sub2_S_NEW_Attach"];
                    j1["S_HORN"] = j1["Sub2_S_HORN"];
                    j1["S_DAY"] = j1["Sub2_S_DAY"];
                  }                
                  this.dslist.push(j1);
                  n1++;
                }
            ...
            created: function () {
              ...
              if (smm == '12') {
                nyy++;
                nmm = 0;
                d1 = new Date(nyy, nmm, 1);
                d2 = new Date(nyy, nmm + 1, 1);
                ss = moment(d2).endOf('month').format('YYYY-MM-DD');
                sdd = ss.substring(8, 10);
                ndd = parseInt(sdd);
                d2 = new Date(nyy, nmm + 1, ndd);
              }
              else {
                d1 = new Date(nyy, nmm, 1);
                d2 = new Date(nyy, nmm + 1, 1);
              ...
              this.timeRange.value1[0] = d1;
              this.timeRange.value1[1] = d2;             
            },         
