...
app.MapPost("/listvvv", async (HttpRequest request) =>
{
    ...
    DataTable dt, dt1;
    MssqlR t1 = new MssqlR();
    v011 = t1.dot2rep(v011);
    v012 = t1.dot2rep(v012);
    //vsql = "select * from p02v";
    //vsql="WITH A01CTE (ALLID,CRTDT,URLPATH,URLPATH1,PRODNAME,AIRLINE,STDATE,WEEKOF,T_DAY,SALES,VISAC,TAXC,TIPC,QTY,SIGNSTS,REM,SALENO) AS";
    ...
    //vsql="select * from p02v where 1=0";
    //dt1= t1.RunSQL(vsql);
    vsql = "";
    //vsql +="SELECT ALLID,CRTDT,URLPATH,URLPATH1,PRODNAME,AIRLINE,STDATE,WEEKOF,T_DAY,SALES,VISAC,TAXC,TIPC,QTY,SIGNSTS,REM,SALENO";
    ...
    //vsql+="),A02CTE (SALENO) AS ( SELECT SALENO FROM A01CTE GROUP BY SALENO)";
    //vsql+=" SELECT PRODNAME,AIRLINE,STDATE,WEEKOF,T_DAY,SALES,VISAC,TAXC,TIPC,QTY,SIGNSTS,REM,SALENO,CRTDT,ALLID,URLPATH,URLPATH1 FROM A01CTE";
    Console.WriteLine(vsql);
    dt = t1.RunSQL(vsql);
    dt1 = dt.Clone();
    DataRow r1;
...
app.MapPost("/listvvv1", async (HttpRequest request) =>
{
    var body = new StreamReader(request.Body);
    string postData = await body.ReadToEndAsync();
    dynamic get_data = JObject.Parse(postData);
